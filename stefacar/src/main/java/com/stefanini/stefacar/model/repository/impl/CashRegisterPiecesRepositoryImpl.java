...
public class CashRegisterPiecesRepositoryImpl extends AbstractRepository<CashRegisterPieces> {

	public boolean thereSaleOn() {
		int listTest = entityManager.createQuery("select l from " + CashRegisterPieces.class.getSimpleName() + " l")
				.getFirstResult();
		if (listTest == 0) {
			return false;
		} else {
...
//		java.sql.Date dataSql = (java.sql.Date) entityManager.createNativeQuery("select dateOfSale from cashregisterpieces")
//				.getSingleResult();
